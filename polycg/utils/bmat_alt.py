...
class BOMat:
    ...
    def extract_coo(
        ...
        """
        COO-emitting counterpart of :meth:`extract`.

        Instead of accumulating into a (dense or sparse) target matrix, append
        this block's contribution over the window ``[x1, x2) x [y1, y2)`` as flat
        coordinate/value arrays to the provided lists. ``rows``/``cols`` hold
        window-local indices, ``vals`` the weighted block entries, and ``cnts``
        the per-entry weights. Duplicate coordinates (from overlapping blocks or
        periodic images) are summed later when the COO triplets are assembled into
        a sparse matrix, exactly reproducing the averaging semantics of
        :meth:`extract`.
        """
        # Non-periodic: simple overlap test
        if not self.periodic:
            if self.x2 <= x1 or self.x1 >= x2 or self.y2 <= y1 or self.y1 >= y2:
                return
            self._extract_coo(rows, cols, vals, cnts, x1, x2, y1, y2, use_weight=use_weight)
            return
    ...
    def _extract_coo(
        ...
        # Mirrors the index arithmetic of _extract, but emits COO triplets.
        xlo = max(self.x1, x1)
        xhi = min(self.x2, x2)
        ylo = max(self.y1, y1)
        yhi = min(self.y2, y2)
        ...
        # Window-local target indices (row-major to match block_vals.ravel())
        tx = np.arange(xlo - x1, xhi - x1)
        ty = np.arange(ylo - y1, yhi - y1)
        rows.append(np.repeat(tx, dy))
        cols.append(np.tile(ty, dx))
        vals.append(np.asarray(block_vals).ravel() * weight)
        cnts.append(np.full(dx * dy, weight, dtype=np.float64))
...
class BlockOverlapMatrix:
    ...
    def to_sparse(self,
        ...
        # Accumulate numerator and counts as COO triplets. Building the CSC
        # matrices directly from triplets (which sum duplicate coordinates) avoids
        # the large per-nonzero overhead of lil_matrix slice-assignment and the
        # transient double-storage during a lil->csc conversion. Numerator (vals)
        # and counts (cnts) share the same (row, col) coordinates, so the overlap
        # averaging is reproduced exactly: averaged = sum(weight*value) / sum(weight).
        rows: list = []
        cols: list = []
        vals: list = []
        cnts: list = []
        ...
        # csc_matrix sums duplicate (row, col) entries on construction.
        S = csc_matrix((vals, (rows, cols)), shape=(nx, ny))
        C = csc_matrix((cnts, (rows, cols)), shape=(nx, ny))
